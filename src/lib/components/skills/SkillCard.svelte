			...
			<div class="flex items-center gap-2">
				...
				{#if skill.source === 'auto-detected'}
					<span
						...
						title={skill.sourcePath ? m.badge_source_path({ path: skill.sourcePath }) : m.badge_auto_detected()}
					>
						{m.badge_auto()}
					</span>
				{/if}
			...
			<div class="flex items-center gap-1.5 mt-2 flex-wrap">
				{#if skill.allowedTools && skill.allowedTools.length > 0}
					<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-600 dark:bg-blue-900/50 dark:text-blue-400">
						{m.badge_tool_count({ count: skill.allowedTools.length })}
					</span>
				{/if}
			...
			<div class="flex items-center gap-1">
				{#if onFavoriteToggle}
					<button
						...
						title={skill.isFavorite ? m.tooltip_remove_favorite() : m.tooltip_add_favorite()}
					>

...
vi.mock('$lib/types', () => {
	...
	const actions = [
		...
		{ action: 'newline', label: 'New Line', description: 'Insert newline', defaultKeys: ['Shift+Enter'], context: 'Chat' }
	];
	return {
...
describe('Keybindings Library Store', () => {
	...
	describe('setBinding', () => {
		...
		it('should create new context block if needed', async () => {
			const { keybindingsLibrary } = await import('$lib/stores/keybindingsLibrary.svelte');
			keybindingsLibrary.setBinding('Chat', 'Ctrl+A', 'newline');
			const block = keybindingsLibrary.overrides.find((b) => b.context === 'Chat');
	...
	describe('context expansion', () => {
		...
		it('should expand all', async () => {
			...
			expect(keybindingsLibrary.expandedContexts.has('Global')).toBe(true);
			expect(keybindingsLibrary.expandedContexts.has('Chat')).toBe(true);
		});
	...
	describe('conflict detection', () => {
		...
		it('should check Global context for non-global contexts', async () => {
			const { keybindingsLibrary } = await import('$lib/stores/keybindingsLibrary.svelte');
			const conflicts = keybindingsLibrary.detectConflicts('Chat', 'Enter');
		...
		it('should check all contexts when context is Global', async () => {
			...
			const inputConflict = conflicts.find((c) => c.context === 'Chat');
	...
	describe('filteredByContext', () => {
		...
		it('should exclude contexts with no matches', async () => {
			...
			expect(filtered.has('Chat')).toBe(true);
		});
	});
});
